...
interface ISearchFiltersProps extends IVideoPlayerProps {
  onChange: (value: string) => void;
  onSubmit: (value: string) => void;
}

export const SearchFilters: React.FC<ISearchFiltersProps> = ({ videoUrl, onChange, onSubmit }) => {
  // Create new state variable for search input
  const [search, setSearch] = React.useState<string>('')

  // Handle change in search input and update search state
  const handleChange = (event: ChangeEvent<HTMLInputElement>) => {
    setSearch(event.target.value)
  }
  // Handle form submission and pass search state to parent component via onSubmit prop
  ...
  return (
    <div className="m-10 lg:w-screen max-w-screen-md">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-lg">
          <form className="" onSubmit={handleSubmit}>
            <div className="relative mb-10 w-full flex  items-center justify-between rounded-md">
              ...
              <input type="text" name="search" value={search} onChange={handleChange} className="h-12 w-full cursor-text rounded-md border border-gray-100 bg-gray-100 py-4 pr-40 pl-12 shadow-sm outline-none focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50" placeholder="Search by name, type, manufacturer, etc" />
            </div>
